defmodule Poxa do
  ...
  def start(_type, _args) do
    ...
    case load_config() do
      {:ok, config} ->
        Logger.info("Starting Poxa via kyrne/websocket, app_id: #{config.app_id}")

        case Application.fetch_env(:poxa, :ssl) do
          {:ok, ssl_config} ->
            if !enabled_ssl?(ssl_config) do
              Logger.info("Starting without SSL on port #{config.port}")
    ...
    case Application.fetch_env(:poxa, :ssl) do
      {:ok, ssl_config} ->
        if enabled_ssl?(ssl_config) do
          ...
          Logger.info("Starting with SSL on port #{ssl_port}")
